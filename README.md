# えんぴつだいあろーぐ。— コンセプト
ほんの数行のテキストを読むため、たった1枚の写真を見たいがため、無数の不要な広告、過剰なアニメーション演出、大きすぎるサイズの画像など、何かとノイズの多い昨今のWebサイトに対し、限りなくシンプルで、容量も少なく、10年後20年後でも「インターネットの片隅に残ってしまうような場所」をコンセプトに作成した、実験的なサイトです。

つぶやきでも、対話でもない、でも同じ空間にゆるく共存してるような、ゆるくつながりが感じられるような空間を目指しています。
...
投稿時間は1時間単位、誰かのあしあとも「ちょっと前」というようなアバウトな表示。「あ、誰かが読んでいたんだな」という気配だけ感じられるように。

### 通信料を記載

右上、「XXX KB」と表示されている箇所に、通信量がわかるウィンドウを置いています。クリックすると、スマホの写真何枚分か、動画が何分見られるか、初代マリオブラザーズ何本分に匹敵する通信量かわかります。パケ死対策もバッチリです。
